...
var address;
var area_title = 'User Management';
$(document).ready(function(){
    ...
    // Define Event
    $('.centra-a-link').click(doOnAdminMenuClick);
    ...
    $('#pair_mng').click(doOnLoadMngPage);
    $('#fee_mng').click(doOnLoadMngPage);
    $('#erc20_mng').click(doOnLoadMngPage);
...
function doOnAdminMenuClick() {
    var menu_prop = $(this).attr('menu-prop');
    var area_title = $(this)[0].childNodes[2].innerHTML;
    $('.centra-panel-heading').html(area_title);
}
...
function reSizeERC20AssetHeight() {
    var total_Height = parseFloat($(window).height())-72;
    $('.centra-scroll').css('height', (total_Height)+'px');
    $('.centra-panel').css('height', (total_Height+20)+'px');
    $('#panel_iframe').css('height', (total_Height-30)+'px');
}
...
function doOnLoadMngPage() {
    url = $(this).attr('chref');
    $('#panel_iframe').attr('src', url);
}
